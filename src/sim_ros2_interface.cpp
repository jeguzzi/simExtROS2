...
public:
  ...
  void onSimulationEnded() { shutdownTransientProxies(); }
  ...
  static void
  ros_imtr_callback(const sensor_msgs::msg::Image::ConstSharedPtr &msg,
  ...
  void createSubscription(createSubscription_in *in,
    ...
    subscriptionProxy->destroyAfterSimulationStop =
        shouldProxyBeDestroyedAfterSimulationStop(in->_scriptID);
...
#include <sub_new.cpp>
  ...
  void shutdownSubscription(shutdownSubscription_in *in,
    ...
    if (subscriptionProxies.find(in->subscriptionHandle) ==
        subscriptionProxies.end()) {
      throw sim::exception("invalid subscription handle");
    }

    SubscriptionProxy *subscriptionProxy =
        subscriptionProxies[in->subscriptionHandle];
...
#include <sub_del.cpp>
  ...
  void createPublisher(createPublisher_in *in, createPublisher_out *out) {
    ...
    publisherProxy->destroyAfterSimulationStop =
        shouldProxyBeDestroyedAfterSimulationStop(in->_scriptID);
...
#include <pub_del.cpp>
  ...
  void
  publisherTreatUInt8ArrayAsString(publisherTreatUInt8ArrayAsString_in *in,
...
#include <pub_publish.cpp>
  ...
  void createClient(createClient_in *in, createClient_out *out) {
    ...
    clientProxy->destroyAfterSimulationStop =
        shouldProxyBeDestroyedAfterSimulationStop(in->_scriptID);
...
#include <cli_call.cpp>
  ...
  void createService(createService_in *in, createService_out *out) {
    ...
    serviceProxy->destroyAfterSimulationStop =
        shouldProxyBeDestroyedAfterSimulationStop(in->_scriptID);
...
#include <srv_del.cpp>
  ...
  void createActionClient(createActionClient_in *in,
    ...
    actionClientProxy->destroyAfterSimulationStop =
        shouldProxyBeDestroyedAfterSimulationStop(in->_scriptID);
...
#include <actcli_new.cpp>
  ...
  void shutdownActionClient(shutdownActionClient_in *in,
    ...
    if (actionClientProxies.find(in->actionClientHandle) ==
        actionClientProxies.end()) {
      throw sim::exception("invalid action client handle");
    }

    ActionClientProxy *actionClientProxy =
        actionClientProxies[in->actionClientHandle];
...
#include <actcli_cancelLastGoal.cpp>
  ...
  void createActionServer(createActionServer_in *in,
    ...
    actionServerProxy->destroyAfterSimulationStop =
        shouldProxyBeDestroyedAfterSimulationStop(in->_scriptID);
...
#include <actsrv_new.cpp>
  ...
  void shutdownActionServer(shutdownActionServer_in *in,
    ...
    if (actionServerProxies.find(in->actionServerHandle) ==
        actionServerProxies.end()) {
      throw sim::exception("invalid action server handle");
    }

    ActionServerProxy *actionServerProxy =
        actionServerProxies[in->actionServerHandle];
...
#include <actsrv_del.cpp>
  ...
  getGoalHandle(ActionServerProxy *actionServerProxy,
                const std::string &goalUUIDstr) {
...
#include <actsrv_action_is_executing.cpp>
    ...
    try {
      geometry_msgs::msg::TransformStamped transform =
          tf_buffer->lookupTransform(in->target_frame, in->source_frame,
                                     tf2::TimePoint());
      ROS2WriteOptions wo;
      write__geometry_msgs__msg__TransformStamped(transform, in->_stackID, &wo);
  ...
  void
  imageTransportCreateSubscription(imageTransportCreateSubscription_in *in,
  ...
  void
  imageTransportShutdownPublisher(imageTransportShutdownPublisher_in *in,
  ...
  void getTime(getTime_in *in, getTime_out *out) {
    ...
    case sim_ros2_clock_ros:
      t = RCL_ROS_TIME;
      break;
    case sim_ros2_clock_system:
      t = RCL_SYSTEM_TIME;
      break;
    case sim_ros2_clock_steady:
      t = RCL_STEADY_TIME;
      break;
    }
...
#include <if_list.cpp>
  ...
  void shutdownTransientSubscriptions() {
    ...
    for (std::map<int, SubscriptionProxy *>::iterator it =
             subscriptionProxies.begin();
         it != subscriptionProxies.end(); ++it) {
  ...
  void shutdownTransientPublishers() {
    ...
    for (std::map<int, PublisherProxy *>::iterator it =
             publisherProxies.begin();
         it != publisherProxies.end(); ++it) {
      if (it->second->destroyAfterSimulationStop) {
        handles.push_back(it->first);
      }
    }
  ...
  void shutdownTransientClients() {
    ...
    for (std::map<int, ClientProxy *>::iterator it = clientProxies.begin();
         it != clientProxies.end(); ++it) {
      if (it->second->destroyAfterSimulationStop) {
        handles.push_back(it->first);
      }
    }
  ...
  void shutdownTransientServices() {
    ...
    for (std::map<int, ServiceProxy *>::iterator it = serviceProxies.begin();
         it != serviceProxies.end(); ++it) {
      if (it->second->destroyAfterSimulationStop) {
        handles.push_back(it->first);
      }
    }
  ...
  void shutdownTransientActionClients() {
    ...
    for (std::map<int, ActionClientProxy *>::iterator it =
             actionClientProxies.begin();
         it != actionClientProxies.end(); ++it) {
      if (it->second->destroyAfterSimulationStop) {
        handles.push_back(it->first);
      }
    }
  ...
  void shutdownTransientActionServers() {
    ...
    for (std::map<int, ActionServerProxy *>::iterator it =
             actionServerProxies.begin();
         it != actionServerProxies.end(); ++it) {
      if (it->second->destroyAfterSimulationStop) {
        handles.push_back(it->first);
      }
    }
